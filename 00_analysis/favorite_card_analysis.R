...
# pluck function - extracts elements of nested list
...
get_stock_mavg_info <- function(data) {
  ...
  data %>%
    tail(1) %>%
    dplyr::mutate(mavg_warning_flag = mavg_short < mavg_long,
                  n_short = n_short,
                  n_long = n_long,
...
generate_favorite_card <- function(data) {
  column(
    ...
    info_card(
      ...
      sub_text_color = ifelse(data$mvg_warning_flag, "danger", "success"),
...
generate_favorite_cards <- function(favorites, 
  ...
  favorites %>%
    ...
    # Step 2
    map(.f = function(data) {
      data %>% get_stock_mavg_info()
    }) %>%
    # Step 3
    dplyr::bind_rows(.id = "stock") %>%
    dplyr::mutate(stock = as_factor(stock)) %>%
    split(.$stock) %>%
    # Step 4
    map(
      .f = function(data) {data %>% generate_favorite_card()}
    ) %>%
    # Step 5
    tagList()
}
  
favorites = c("NVDA", "AAPL")
generate_favorite_cards(favorites)
