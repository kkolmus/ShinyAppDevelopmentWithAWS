...
# static table, normally it would have been stored in a database
...
ui <- tagList(
  ...
  shinyauthr::loginUI(
    ...
    title = tagList(
      h1(
        class = "text-center", "Stock Analyzer"),
      p(
        class = "text-center", "Please log in")
    )
    # you can manipulate text for username, password and login button
  ),
...
server <- function(input, output, session) {
  ...
  # 0.1 credentials ----
  # it is a package, if it is a script than you need to source it first
  ...
  observe(
    {
      if (credentials()$user_auth) {
        reactive_values$permissions <- credentials()$info$permissions
        reactive_values$user_name <- credentials()$info$name
        reactive_values$favorites_list <- credentials()$info %>% 
          pull(favorites) %>% 
          pluck(1)
        reactive_values$last_symbol <- credentials()$info$last_symbol
      }
    }
  )
  ...
  # moved to the section related to user authenticaltion 
  # reactive_values <- reactiveValues()
            ...
            wellPanel(
              div(
                ...
                pickerInput(
                  ...
                  multiple = FALSE, 
                  selected = stock_list_tbl %>% filter(
                    label %>% 
                      str_detect(paste0(reactive_values$last_symbol, ","))) %>% pull(label),
